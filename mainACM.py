...
def load_dataset():
    ...
    paper_global_idx = list(range(0, 3025))  # 0~3024 : paper nodes
    author_global_idx = list(range(3025, 8937))  # 3025~8936 : author nodes
    subject_global_idx = list(range(8937, 8994))  # 8967~8993 # subject nodes

    num_nodes_dict = {
        'P': len(paper_global_idx),
        'A': len(author_global_idx),
        'S': len(subject_global_idx)
    }
    # index mapping dictionary
    gid2lid = {}
    for i, gid in enumerate(paper_global_idx):
        gid2lid[gid] = ('P', i)
    for i, gid in enumerate(author_global_idx):
        gid2lid[gid] = ('A', i)
    for i, gid in enumerate(subject_global_idx):
        gid2lid[gid] = ('S', i)
    ...
    add_edges(edge_list[0], 'P', 'P-A', 'A')
    add_edges(edge_list[1], 'A', 'A-P', 'P')
    add_edges(edge_list[2], 'P', 'P-S', 'S')
    add_edges(edge_list[3], 'S', 'S-P', 'P')
    ...
    for ntype in G.ntypes:
        if ntype == 'A':
            G.nodes[ntype].data['feats'] = torch.Tensor(node_features[author_global_idx])
        elif ntype == 'P':
            G.nodes[ntype].data['feats'] = torch.Tensor(node_features[paper_global_idx])
        elif ntype == 'S':
            G.nodes[ntype].data['feats'] = torch.Tensor(node_features[subject_global_idx])
        else:
...
def run_model_ACM():
    ...
    for iter in range(5):
        net = HypHetFormer(hete_G, hete_G.nodes['P'].data['feats'].size(1), hidden_dim, out_dim, num_relations, num_class, device=device, is_large=False)
        net.to(device)
        ...
        early_stopping = EarlyStopping(patience=patience, verbose=True, save_path="checkpoint/checkpoint_ACM_"+str(train_percent)+".pt")
        ...
        for epoch in range(epochs):
            ...
            if early_stopping.early_stop:
                ...
                break;
        net.load_state_dict(torch.load("checkpoint/checkpoint_ACM_"+str(train_percent)+".pt"))
        net.eval()
        with torch.no_grad():
            logits, embeddings = net(hete_G, out_type)
