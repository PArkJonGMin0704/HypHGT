...
def load_dataset():
    ...
    author_global_idx = list(range(0, 4057))  # 0~4056 : author nodes
    paper_global_idx = list(range(4057, 18385))  # 4057~18384 : paper nodes
    conf_global_idx = list(range(18385, 18405))  # 18385~18404 # conference nodes

    num_nodes_dict = {
        'A': len(author_global_idx),
        'P': len(paper_global_idx),
        'C': len(conf_global_idx)
    }
    ...
    for i, gid in enumerate(paper_global_idx):
        gid2lid[gid] = ('P', i)
    for i, gid in enumerate(conf_global_idx):
        gid2lid[gid] = ('C', i)
    ...
    add_edges(edge_list[2], 'P', 'P-C', 'C')
    add_edges(edge_list[3], 'C', 'C-P', 'P')
    ...
    for ntype in G.ntypes:
        if ntype == 'A':
            ...
            G.nodes[ntype].data['feats'] = torch.Tensor(node_features[conf_global_idx])
        else:
...
def run_model_DBLP():
    ...
    for iter in range(5):
        net = HypHetFormer(hete_G, hete_G.nodes['A'].data['feats'].size(1), hidden_dim, out_dim, num_relations, num_class, device=device, is_large=False)
        net.to(device)
        ...
        early_stopping = EarlyStopping(patience=patience, verbose=True, save_path="checkpoint/checkpoint_DBLP_" + str(train_percent) + ".pt")
        ...
        for epoch in range(epochs):
            ...
            if early_stopping.early_stop:
                ...
                break;
        net.load_state_dict(torch.load("checkpoint/checkpoint_DBLP_" + str(train_percent) + ".pt"))
        net.eval()
        with torch.no_grad():
            logits, embeddings = net(hete_G, out_type)
